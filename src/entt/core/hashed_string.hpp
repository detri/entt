...
template<class T>
concept byte_like =
    std::same_as<std::remove_cv_t<T>, char> || std::same_as<std::remove_cv_t<T>, signed char> || std::same_as<std::remove_cv_t<T>, unsigned char> || std::same_as<std::remove_cv_t<T>, std::byte>;
...
struct wchar_byte_ptr {
    ...
    [[nodiscard]]
    constexpr std::uint8_t operator[](
        const std::size_t index) const noexcept {
        using unsigned_wchar = std::make_unsigned_t<wchar_t>;

        const std::size_t byte_index = offset + index;
        const std::size_t char_index =
            byte_index / sizeof(wchar_t);
        const std::size_t char_byte =
            byte_index % sizeof(wchar_t);
...
[[nodiscard]] constexpr std::uint64_t read32_constexpr(
    ...
    return std::uint64_t{byte_value(p[0])} | (std::uint64_t{byte_value(p[1])} << 8) | (std::uint64_t{byte_value(p[2])} << 16) | (std::uint64_t{byte_value(p[3])} << 24);
}

template<byte_like Byte>
[[nodiscard]] constexpr std::uint64_t read64_constexpr(
    const Byte *p) noexcept {
    return std::uint64_t{byte_value(p[0])} | (std::uint64_t{byte_value(p[1])} << 8) | (std::uint64_t{byte_value(p[2])} << 16) | (std::uint64_t{byte_value(p[3])} << 24) | (std::uint64_t{byte_value(p[4])} << 32) | (std::uint64_t{byte_value(p[5])} << 40) | (std::uint64_t{byte_value(p[6])} << 48) | (std::uint64_t{byte_value(p[7])} << 56);
}
...
[[nodiscard]]
RAPIDHASH_ALWAYS_INLINE constexpr std::uint64_t read32(
    const wchar_byte_ptr p) noexcept {
    return std::uint64_t{p[0]} | (std::uint64_t{p[1]} << 8u) | (std::uint64_t{p[2]} << 16u) | (std::uint64_t{p[3]} << 24u);
}

[[nodiscard]]
RAPIDHASH_ALWAYS_INLINE constexpr std::uint64_t read64(
    const wchar_byte_ptr p) noexcept {
    return std::uint64_t{p[0]} | (std::uint64_t{p[1]} << 8u) | (std::uint64_t{p[2]} << 16u) | (std::uint64_t{p[3]} << 24u) | (std::uint64_t{p[4]} << 32u) | (std::uint64_t{p[5]} << 40u) | (std::uint64_t{p[6]} << 48u) | (std::uint64_t{p[7]} << 56u);
}
...
RAPIDHASH_ALWAYS_INLINE constexpr void mum(
    ...
    apply_product(lhs, rhs, {static_cast<std::uint64_t>(product), static_cast<std::uint64_t>(product >> 64)});
...
[[nodiscard]]
constexpr std::uint64_t rapidhash_with_seed(
    const wchar_t *key,
    const std::size_t len,
    const std::uint64_t seed_value) noexcept {
    return detail::hash_internal<detail::variant::full>(
...
[[nodiscard]]
constexpr std::uint64_t rapidhash_micro_with_seed(
    const wchar_t *key,
    const std::size_t len,
    const std::uint64_t seed_value) noexcept {
    return detail::hash_internal<detail::variant::micro>(
...
[[nodiscard]]
constexpr std::uint64_t rapidhash_nano_with_seed(
    const wchar_t *key,
    const std::size_t len,
    const std::uint64_t seed_value) noexcept {
    return detail::hash_internal<detail::variant::nano>(
...
[[nodiscard]]
constexpr std::uint64_t rapidhash(
    const wchar_t *key,
    const std::size_t len) noexcept {
...
[[nodiscard]]
constexpr std::uint64_t rapidhash_micro(
    const wchar_t *key,
    const std::size_t len) noexcept {
...
[[nodiscard]]
constexpr std::uint64_t rapidhash_nano(
    const wchar_t *key,
    const std::size_t len) noexcept {
...
[[nodiscard]] constexpr std::uint64_t rapidhash(const std::wstring_view value) noexcept {
    return rapidhash(value.data(), value.size());
}

[[nodiscard]] constexpr std::uint64_t rapidhash_micro(const std::wstring_view value) noexcept {
    return rapidhash_micro(value.data(), value.size());
}

[[nodiscard]] constexpr std::uint64_t rapidhash_nano(const std::wstring_view value) noexcept {
    return rapidhash_nano(value.data(), value.size());
}
...
template<std::size_t N>
[[nodiscard]] constexpr std::uint64_t rapidhash(const wchar_t (&value)[N]) noexcept {
    static_assert(N > 0);
    return rapidhash(value, N - 1);
}

template<std::size_t N>
[[nodiscard]] constexpr std::uint64_t rapidhash_micro(const wchar_t (&value)[N]) noexcept {
    static_assert(N > 0);
    return rapidhash_micro(value, N - 1);
}

template<std::size_t N>
[[nodiscard]] constexpr std::uint64_t rapidhash_nano(const wchar_t (&value)[N]) noexcept {
    static_assert(N > 0);
    return rapidhash_nano(value, N - 1);
}
...
template<>
struct basic_string_hash<stl::uint64_t> {
    template<typename Char>
    [[nodiscard]] static constexpr stl::uint64_t value(
        const Char *str,
        const stl::size_t len) noexcept
    {
        return rapidhash::rapidhash_micro(str, len);
    }
};

template<>
struct basic_string_hash<stl::uint32_t> {
    template<typename Char>
    [[nodiscard]] static constexpr stl::uint32_t value(
        const Char *str,
        const stl::size_t len) noexcept
    {
        const stl::uint64_t hash =
            rapidhash::rapidhash_micro(str, len);
...
struct basic_hashed_string {
    ...
    using hasher = internal::basic_string_hash<>;

    const value_type *repr{};
    hash_type hash{hasher::value("", 0)};
    size_type length{};
};
...
public:
    ...
    constexpr basic_hashed_string(
        ...
        : base_type{
        str,
        base_type::hasher::value(str, len),
        len
    }
    {}
    ...
    ENTT_CONSTEVAL basic_hashed_string(const value_type (&str)[N]) noexcept
        : base_type{str}
    {
        while(base_type::length < N && str[base_type::length] != value_type{}) {
            ++base_type::length;
        }

        base_type::hash = base_type::hasher::value(str, base_type::length);
    }
    ...
    {
        while(wrapper.repr[base_type::length] != value_type{}) {
            ++base_type::length;
        }

        base_type::hash = base_type::hasher::value(
                wrapper.repr,
                base_type::length);
    }
